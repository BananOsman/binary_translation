...
#define start_loop_inst_addr 0x409fde
#define addl_inst_addr 0x40a069
#define mov_inst_addr 0x40a06d
	...
	for (int i=0; i < num_of_instr_map_entries; i++)
	{
		...
		if((tc_targ_addr == instr_map[i].new_ins_addr + instr_map[i].size) && (instr_map[i].iclass_enum == XED_ICLASS_JL))
		{
			instr_map[i].hasNewTargAddr = true;
	            	instr_map[i].new_targ_entry = imp_points[1];
			extension = true;
			dump_instr_map_entry(i);
               		continue;
		} else if (instr_map[i].iclass_enum == XED_ICLASS_JL && extension && jl)
		{
			instr_map[i].hasNewTargAddr = true;
	            	instr_map[i].new_targ_entry = imp_points[0];
			jl = false;
			dump_instr_map_entry(i);
               		continue;
		...
		{
			instr_map[i].hasNewTargAddr = true;
	            	instr_map[i].new_targ_entry = imp_points[2];
			dump_instr_map_entry(i);
               		continue;
		}
...
int encode_sub_inst()
{
	...
	xed_code = xed_decode(&xedd, reinterpret_cast<UINT8*>(addr), ilen);
	if (xed_code != XED_ERROR_NONE) {
	...
	if (xed_code != XED_ERROR_NONE) {
		cerr << "ERROR: xed decode failed the new mov!"<< endl;
		return -1;
	}
	...
	if (!convert_ok) {
        ...
        xed_error = xed_encode(&enc_req, reinterpret_cast<UINT8*>(&encoded_ins), ilen, &olen);
	if (xed_error != XED_ERROR_NONE) {
		cerr << "ENCODE ERROR: " << xed_error_enum_t2str(xed_error) << endl;
		return -1;
	}
	...
	xed_code = xed_decode(&xedd, reinterpret_cast<UINT8*>(encoded_ins), max_inst_len);
	if (xed_code != XED_ERROR_NONE) {
		cerr << "ERROR: xed decode failed the new add!"<< endl;
		return -1;
	}

	InsSize = xed_decoded_inst_get_length(&xedd);
	rc = add_new_instr_entry(&xedd, 0, InsSize);
	if (rc < 0) {
		cerr << "ERROR: failed during instructon translation." << endl;
		translated_rtn[translated_rtn_num].instr_map_entry = -1;
...
int encode_con_jmp_inst()
{
	xed_decoded_inst_t xedd;
	xed_error_enum_t xed_code;
	ADDRINT addr = end_loop_inst_addr;
	...
	for (int i=0; i<4; i++)
	{
		if(i == 0)
		{
			xed_inst2(&enc_instr, dstate, XED_ICLASS_MOV, 32, xed_reg(XED_REG_EAX),
					xed_mem_bd(XED_REG_RBP, xed_disp(-0x18, 32), 32));
		...
		xed_inst2(&enc_instr, dstate, XED_ICLASS_ADD, 32, xed_reg(XED_REG_EAX),xed_imm0(4,32));
		} else if(i == 2)
		{
			//xed_inst2(&enc_instr, dstate, XED_ICLASS_ADD, 32, xed_reg(XED_REG_ESI),xed_imm0(-4,32));
			xed_inst2(&enc_instr, dstate, XED_ICLASS_CMP, 32, xed_reg(XED_REG_EAX), xed_mem_bd(XED_REG_RBP,xed_disp(-0x86c,32),32));
		}else if(i == 3)
		{
...
int add_fallBackSort(IMG img)
{
	int rc;
	//adding fallbackSort
	RTN rtn = RTN_FindByName(img, "fallbackSort");
	if (rtn == RTN_Invalid()) {
		cerr << "Warning: invalid routine fallbackSort" << endl;
		return -1;
	}
	int Unrolling = 1;
	bool add_reminant = false;
	INS loop_start_ins;
	bool end_loop = false;
	int ExtSize = 0;
	...
	int size = 0,InsSize;
	if (KnobVerbose) {
		cerr << "rtn name: " << RTN_Name(rtn) << " : " << dec
				<< translated_rtn_num << endl;
	}

	//to translate routine
	translated_rtn[translated_rtn_num].rtn_addr = RTN_Address(rtn);
	translated_rtn[translated_rtn_num].rtn_size = RTN_Size(rtn)+45*4;
	...
	for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
	{
		...
		if (addr == start_loop_inst_addr && add_extension)
		{
			if((ExtSize = decode_extension())<0)
			{
				cerr << "Error: adding extension failed!" << endl;
				return -1;
			}
			loop_start_ins = INS_Prev(ins);
			add_extension = false;
		}
		xed_decoded_inst_t xedd;
		xed_error_enum_t xed_code;
		...
		if ((addr == mov_inst_addr) && (Unrolling < 4))
		{
			ins = loop_start_ins;
			Unrolling++;
			add_reminant = true;
		...
		if((addr == end_loop_inst_addr) && add_reminant == true)
		{
			add_reminant = false;
			int tmp_size;
			...
			end_loop = true;
			ins = loop_start_ins;
		} else if ((addr == end_loop_inst_addr) && (end_loop == true))
		{
			imp_points[2] = num_of_instr_map_entries;
			end_loop = false;
		}
...
VOID rtnWrapper(RTN rtn, VOID *v)
{
	...
	rtnInfo->imgName = IMG_Name(SEC_Img(RTN_Sec(rtn)));
...
{
	return (rtn1->rtnInvCount > rtn2->rtnInvCount);
}

VOID Fini(INT32 code, VOID *v)
{
	ofstream outFile;
	outFile.open("hot-routines.csv");
	vector <rtn_ent*> rtn_vec;
    for(list<rtn_ent*>::iterator it = rtn_list.begin(); it != rtn_list.end(); ++it )
    {
    	rtn_vec.push_back(*it);
	}
	sort(rtn_vec.begin(),rtn_vec.end(),compare);

	for(vector<rtn_ent*>::iterator it=rtn_vec.begin(); it!=rtn_vec.end(); ++it)
	{
		if((*it)->rtnInvCount != 0)
		{
			outFile << (*it)->rtnName << endl;
		}
	}
