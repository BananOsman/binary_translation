...
KNOB<INT> KnobOpt(KNOB_MODE_WRITEONCE,    "pintool",
    "opt", "0", "Opt run");
...
{
	return (lp1->loopAddrTar < lp2->loopAddrTar);
}
...
VOID Fini(INT32 code, VOID *v)
{
	...
	for(vector<loop_ent*>::iterator it=loop_vec.begin(); it!=loop_vec.end(); ++it)
	{
		if((*it)->countSeen != 0)
		{
			...
			outFile << hex<<(*it)->loopAddrTar << ",";
			outFile << hex<<(*it)->loopAddrJmp << ",";
...
int allocate_and_init_memory(IMG img)
{
	...
	max_ins_count *= 32; // estimating that the number of instructions of the in-lined functions will not exceed the total number of the entire code.
...
int find_params(INS insJmp,xed_int64_t* paramArr)
{
	...
	{
		return 1;
	} else if (regVec.size() == 0 || offsetVec.size() == 0)
	{
		return 1;
	}
	xed_int64_t disp;
	rc = get_val(INS_Prev(insCmp),regVec[0],&disp);
	if(rc < 0)
	{
	...
	{
		cerr<<"mov is not found!"<<endl;
		return 1;
	}
	offsetVec.push_back(disp);

	//all offsets!
	//first offset n and the second is i
	/// new mohammad
...
int decode_extension(xed_int64_t* paramArr)
{
	...
	for (int i=0; i<4; i++)
	{
		...
		{
		xed_inst2(&enc_instr, dstate, XED_ICLASS_ADD, 32, xed_reg(XED_REG_EAX),xed_imm0(KnobOpt,32));
		} else if(i == 2)
		{
...
void elemenate_internal_loops(vector<ADDRINT*>& vec)
{
	for(unsigned int i=0; i<vec.size();i++)
	{
		for(unsigned int j=0; j<vec.size();j++)
		{
			if((vec[i][0]<vec[j][0] || vec[i][0]==vec[j][0]) && vec[j][1]<vec[i][1])
			{
				//j internal
				vec[i][0] = -1;
				vec[i][1] = -1;
			...
			if((vec[i][0]<vec[j][0] || vec[i][0]==vec[j][0]) && vec[j][0]<vec[i][1] && vec[i][1]<vec[j][1])
			{
				vec[j][0] = -1;
				vec[j][1] = -1;
				vec[i][0] = -1;
...
int get_loops_in_func(map<string,vector<ADDRINT*> >& mapFuncLoop)
{
    ifstream inFile;
    inFile.open("hot-loops.csv");
    string csvLoopInfo;

	while (getline(inFile,csvLoopInfo))
	{
		string rtnName = strtok ((char*)csvLoopInfo.c_str(),",");
		char* startAddr = strtok (NULL, ",");
		char* endAddr = strtok (NULL, ",");

		map<string,vector<ADDRINT*> >::iterator itr = mapFuncLoop.find(rtnName);
		if(itr == mapFuncLoop.end())
		{
			vector<ADDRINT*> vec;
			mapFuncLoop.insert(pair<string,vector<ADDRINT*> >(rtnName, vec));
			itr = mapFuncLoop.find(rtnName);
		}
		ADDRINT* addrs = new ADDRINT[2];
		addrs[0] = AddrintFromString(startAddr);
		addrs[1] = AddrintFromString(endAddr);
		itr->second.push_back(addrs);
	}
	inFile.close();


	for(map<string,vector<ADDRINT*> >::iterator itr=mapFuncLoop.begin(); itr!=mapFuncLoop.end(); itr++)
	{
		elemenate_internal_loops(itr->second);
	}
	return 0;
}

int get_start(ADDRINT addr, vector<ADDRINT*>& addrsVec)
{
	for(unsigned int i=0; i<addrsVec.size(); i++)
	{
		if(addrsVec[i][0] == addr)
		{
			return i;
		}
	}
...
int elem_not_ptn_func(IMG img,map<string,vector<ADDRINT*> >& mapFuncLoop, vector<sortEnt>& Sort)
{
    for(map<string,vector<ADDRINT*> >::iterator itr=mapFuncLoop.begin(); itr!=mapFuncLoop.end(); itr++)
    {
    	if(strcmp(itr->first.c_str(),"sendMTFValues")==0 || strcmp(itr->first.c_str(),"__libc_csu_init")==0)
    	{
    		mapFuncLoop.erase(itr);
    		itr=mapFuncLoop.begin();
    		continue;
    	}

    	RTN rtn = RTN_FindByName(img, itr->first.c_str());
    	if (rtn == RTN_Invalid())
    	{
    		cerr << "Warning: invalid routine " << itr->first << endl;
    		continue;
    	}
        ...
        for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
        {
			...
			if(get_start(addr,itr->second) > -1)
			{
				i = get_start(addr,itr->second);
			} else if (i>-1 && (addr == itr->second[i][1]))
			{
	...
	unsigned int org_size = mapFuncLoop.size();
	for(unsigned int l=0; l<org_size;l++)
    {
		map<string,vector<ADDRINT*> >::iterator itrS=mapFuncLoop.begin();
   		for(map<string,vector<ADDRINT*> >::iterator itr=mapFuncLoop.begin(); itr!=mapFuncLoop.end(); itr++)
    	{

			if(itr->second[0][0]<itrS->second[0][0])
			{
				itrS = itr;
			}
    	}
...
{
	int rc;
    map<string,vector<ADDRINT*> > mapFuncLoop;
    if(get_loops_in_func(mapFuncLoop) < 0)
    {
    	cerr<<"can't get loops!!!"<<endl;
    	return -1;
    }
	vector<sortEnt> Sort;
    if(elem_not_ptn_func(img,mapFuncLoop,Sort) < 0)
    {
    	cerr<<"can't get loops!!!"<<endl;
    	return -1;
    }

    for(vector<sortEnt>::iterator itr=Sort.begin(); itr!=Sort.end(); itr++)
    {
    	RTN rtn = RTN_FindByName(img, itr->name.c_str());
    	if (rtn == RTN_Invalid())
    	{
    		cerr << "Warning: invalid routine " << itr->name << endl;
    		//continue;
    	}
        ...
        for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins))
        {
			...
			if(get_start(addr,itr->vec) > -1)
			{
				i = get_start(addr,itr->vec);
				cerr<<"first addr!"<<hex<<addr<<endl;
			...
			} else if (i>-1 && (addr == itr->vec[i][1]) && (unrolling == true)) //jump addr
			{
				rc = find_params(ins,paramArr);
				if(rc == -1)
				{
			...
			if(addr == INS_Address(INS_Prev(cmpIns)) && unrollCount < KnobOpt && (unrolling == false))
			{
				ins = loopStartIns;
				unrollCount++;
				continue;
			...
			if(add_ins)
			{
				cerr<<dec<<num_of_instr_map_entries<<"old addr: "<<hex<<addr<<endl;
			...
			if (i>-1 && addr == itr->vec[i][1]) //jump addr
			{
				...
				} else
				{
					...
					for(int j=0; j<3; j++)
					{
						lp_jmps[INS_Address(INS_Next(loopStartIns))][j] = imp_points[j];
					}
					unrolling = true;
					unrollCount = -1;
				}
...
int chain_all_direct_br_and_call_target_entries()
{
	...
	map<ADDRINT,int*>::iterator itr=lp_jmps.begin();
		...
		for (int i=0; i < num_of_instr_map_entries; i++)
		{
			...
			if (instr_map[i].hasNewTargAddr)
				continue;
			cerr<<dec<<i<<endl;
			ADDRINT tc_targ_addr = 0;
			if (extension)
			{
				tc_targ_addr = instr_map[i].new_ins_addr + instr_map[i].size + instr_map[i].disp;
			...
			for (int j = 0; j < num_of_instr_map_entries; j++)
			{
				...
				|| (extension && (instr_map[j].orig_ins_addr == instr_map[i].orig_targ_addr) && (instr_map[j].orig_ins_addr > instr_map[itr->second[1]-2].orig_ins_addr)))
				{
						cerr<<"ext!"<<endl;
						instr_map[i].hasNewTargAddr = true;
